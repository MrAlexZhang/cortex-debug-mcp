...
export async function configPin(
  ...
  if (mode === 'af') {
    ...
    await setBits(afReg, 0xF << afBit, af << afBit);
    logger.debug(`configPin: P${port}${pin} → AF${af}`);
  }
  logger.debug(`configPin: P${port}${pin} mode=${mode} pull=${pull} speed=${speed} otype=${otype}`);
}
...
export async function initPwm(
  ...
  // For advanced timers (TIM1), MOE must be set in BDTR
  if (tim === 'TIM1') {
