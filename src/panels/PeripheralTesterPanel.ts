...
import { ChipDef, getChip, listChips, gpioBase, GPIO, GPIO_F1, parsePin, lookupAF, USART_REG, USART_REG_LEGACY, CAN_REG, RTC_REG } from '../peripherals/chipMap';
import {
...
export class PeripheralTesterPanel {
  ...
  private async _detectChip(): Promise<void> {
    ...
    let targetsOutput = '';
    try { targetsOutput = await openocdSend('targets'); } catch { /* ignore */ }
  ...
  private async _initGpio(msg: Record<string, unknown>): Promise<void> {
    ...
    const pinStr = (msg.pin as string).toUpperCase();          // e.g. 'PA5'
    ...
    this._send('result', 'initGpio', { ok: true, pin: pinStr, mode, pull });
  }
  ...
  private async _readGpio(msg: Record<string, unknown>): Promise<void> {
    ...
    const idrOffset = chip.gpioModel === 'f1' ? GPIO_F1.IDR : GPIO.IDR;
    const idr = await mdw1(base + idrOffset);

    const pins: Record<string, string> = {};
    for (let i = 0; i < 16; i++) {
      pins[`P${port}${i}`] = (idr >> i) & 1 ? 'HIGH' : 'LOW';
    }
    this._send('result', 'readGpio', { ok: true, port: `GPIO${port}`, idr: `0x${idr.toString(16).padStart(4,'0')}`, pins });
  }
  ...
  private async _initPwm(msg: Record<string, unknown>): Promise<void> {
    const chip = this._chip(msg.chip as string);
    await initPwm(chip, msg.timer as string, {
      pin: msg.pin as string,
      channel: parseInt(msg.channel as string) as 1 | 2 | 3 | 4,
      frequencyHz: parseInt(msg.frequency as string),
      dutyPercent: parseInt(msg.duty as string),
    });
    this._send('result', 'initPwm', { ok: true, timer: msg.timer, pin: msg.pin, frequency: msg.frequency, duty: msg.duty });
  }
  ...
  private async _haltTarget(): Promise<void> {
    const resp = await openocdSend('halt');
    this._send('result', 'haltTarget', { ok: true, response: resp.trim() });
  }
  ...
  /** Generate a <select> with PA0–PF15 grouped by port. */
  private _pinSelect(id: string, defaultPin: string, includeNone = false): string {
    const ports = ['A','B','C','D','E','F','G','H'];
    let html = `<select id="${id}">`;
    if (includeNone) html += `<option value="">— None —</option>`;
    for (const p of ports) {
      html += `<optgroup label="GPIO${p}">`;
      for (let n = 0; n < 16; n++) {
        const pin = `P${p}${n}`;
        html += `<option${pin === defaultPin ? ' selected' : ''}>${pin}</option>`;
      }
      html += `</optgroup>`;
    }
    html += `</select>`;
...
<div id="pane-gpio" class="tab-pane active">
  ...
  <div class="btn-row">
    <button id="btn-init-gpio">Init pin</button>
  </div>

  <div class="section-title">Set output</div>
  <div class="form-grid">
    <label>Pin</label>
    ${this._pinSelect('gpio-set-pin', 'PA5')}
  </div>
  <div class="btn-row">
  ...
  <div class="btn-row">
    <button id="btn-read-gpio">Read IDR</button>
  </div>
</div>
...
<div id="pane-spi" class="tab-pane">
  ...
  <div class="form-grid">
    ...
    <select id="spi-periph"><option>SPI1</option><option>SPI2</option></select>
    <label>SCK pin</label>
  ...
  <div class="form-grid">
    ...
    <label>CS pin</label>
    ${this._pinSelect('spi-tx-cs', 'PA15', true)}
  </div>
  <div class="btn-row">
...
<div id="pane-can" class="tab-pane">
  ...
  <div class="form-grid">
    ...
    <input id="can-psc" value="4" type="number" min="1" max="1024"
      title="CAN_BTR prescaler. BaudRate = fAPB1 / (PSC × (1+BS1+BS2))">
    <label>BS1 segments</label>
    <input id="can-bs1" value="13" type="number" min="1" max="16">
...
<div id="pane-pwm" class="tab-pane">
  ...
  <div class="form-grid">
    ...
    <select id="pwm-timer">
      ...
      <option>TIM14</option><option>TIM16</option><option>TIM17</option>
    </select>
    <label>Channel</label>
    <select id="pwm-ch">
...
btn('btn-spi-transfer').addEventListener('click', () => {
  send({ command:'spiTransfer', chip:currentChip, peripheral:v('spi-periph'),
    cs:v('spi-tx-cs'), data:v('spi-data') });
});
